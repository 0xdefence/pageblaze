...
DATABASE_URL="$DB_URL_VAL" npm run dev:api > /tmp/pageblaze-api-day4.log 2>&1 & API_PID=$!
DATABASE_URL="$DB_URL_VAL" npm run dev:worker > /tmp/pageblaze-worker-day4.log 2>&1 & WORKER_PID=$!
for i in {1..40}; do
  if curl -fsS "$API_URL/healthz" >/dev/null 2>&1; then break; fi
  sleep 1
done

echo "[3/5] Create two snapshots for same URL"
for n in 1 2; do
...
echo "[4/5] Verify visual endpoints"
SN="$(curl -fsS -H "x-api-key: $API_KEY_VAL" "$API_URL/v1/visual/snapshots?limit=5")"
SC="$(printf '%s' "$SN" | node -e "const d=JSON.parse(require('fs').readFileSync(0,'utf8'));process.stdout.write(String((d.snapshots||[]).length));")"
[[ "$SC" -ge 2 ]] || { echo "expected >=2 snapshots"; echo "$SN"; exit 1; }

DF="$(curl -fsS -H "x-api-key: $API_KEY_VAL" "$API_URL/v1/visual/diffs?limit=5")"
DC="$(printf '%s' "$DF" | node -e "const d=JSON.parse(require('fs').readFileSync(0,'utf8'));process.stdout.write(String((d.diffs||[]).length));")"
[[ "$DC" -ge 2 ]] || { echo "expected >=2 diffs"; echo "$DF"; exit 1; }

echo "[5/5] Stats include visual counters"
ST="$(curl -fsS -H "x-api-key: $API_KEY_VAL" "$API_URL/v1/stats")"
printf '%s\n' "$ST" | sed -n '1,120p'

echo "PASS: day4 visual smoke"
